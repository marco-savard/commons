...
public class AntiqueCreatorDemo {
    ...
    public static void main(String[] args) {
        ...
        //set output file
        String basename = "antiquite-" + DateTimeFormatter.ofPattern("yyyy-MM-dd").format(date);
        ...
        try (OutputStream output = new FileOutputStream(outputFile)) {
            ...
            printTitle(document);
            printChart(document, date, display);
            printRomanDate(document, date, display);
            printAtticDate(document, date, display);
            printEphemerides(document, date, display);
            printCitations(document, random);
    ...
    private static void printChart(XWPFDocument document, LocalDate date, Locale display) {
    ...
    private static void printDivinityOfTheDay(XWPFTableCell cell, LocalDate date, Locale display) {
    ...
    private static void printZodiacSign(XWPFTableCell cell, LocalDate date, Locale display) {
    ...
    private static void printRomanDate(XWPFDocument document, LocalDate date, Locale locale) {
        ...
        run.addCarriageReturn();
        run.setText("  " + romanDate.getDisplayName(TextStyle.FULL, locale));
        run.addCarriageReturn();
    }

    private static void printAtticDate(XWPFDocument document, LocalDate date, Locale display) {
    ...
    private static void printEphemerides(XWPFDocument document, LocalDate date, Locale display) {
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("d MMMM", display);
        ...
        for (HistoricalEvent event : events) {
            printEphemeride(run, event, display);
        }
    }

    private static void printEphemeride(XWPFRun run, HistoricalEvent event, Locale display) {
    ...
    private static List<HistoricalEvent> findComingAntiqueEvents(LocalDate date) {
    ...
    private static void printCitations(XWPFDocument document, Random random) {
       ...
       // List<CitationReader.Author> authors = new ArrayList<>(citationsByAuthor.values());
        LocalDate fallOfRome = LocalDate.of(476, Month.SEPTEMBER, 4);
        ...
        for (String name : authors) {
            ...
            printCitation(run, author, citation);
        }
    }

    private static void printCitation(XWPFRun run, CitationReader.Author author, String citation) {
