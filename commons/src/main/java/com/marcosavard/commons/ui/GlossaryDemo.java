...
public class GlossaryDemo {
	...
	private static void addTranslation(UIDefaults defaults, String key, Map<Locale, String> translations,
			Locale locale) {
	...
	private static void printCompare(UIDefaults defaults, String key, String original, Locale locale) {
		String translation = sanitize(defaults.getString(key, locale));
		String line = MessageFormat.format("  {0} : {1}", translation, Levenshtein.distanceOf(original, translation)); 
	...
	private static List<String> getTextProperties(List<Object> properties) {
		List<String> list = properties.stream() //
			.filter(k -> k.toString().toLowerCase().endsWith("text") || //
