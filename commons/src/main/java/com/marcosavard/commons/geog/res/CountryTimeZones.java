...
public class CountryTimeZones {
	...
	private CountryTimeZones(String countryCode) {
		...
		List<TimeZoneEntry> entries = rows.stream().filter(e -> e.country.equals(countryCode)).collect(Collectors.toList()); 
	...
	private Locale findLocale(String countryCode) {
		...
		Locale locale = locales.stream().filter(l -> l.getCountry().equalsIgnoreCase(countryCode)).findFirst().orElse(null);
		return locale;
	}
