...
public class EducationalNetwork {
	...
	public List<UniversitySystem> getUniversitySystems() {
		...
		List<UniversitySystem> universitySystems = organizations.stream().filter(p).map(o -> (UniversitySystem)o).collect(Collectors.toList()); 
	...
	public List<University> getUniversities() {
		...
		List<University> universities = organizations.stream().filter(p).map(o -> (University)o).collect(Collectors.toList()); 
		return universities;
	}
	...
	public List<University> getUniversitiesByRegion(RegionAdministrative region) {
		...
		List<University> universities = allUniversities.stream().filter(u -> u.getRegion().equals(region)).collect(Collectors.toList()); 
		return universities;
	}
	...
	public List<College> getColleges() {
		...
		List<College> colleges = organizations.stream().filter(p).map(o -> (College)o).collect(Collectors.toList()); 
		return colleges;
	}
	
	public List<College> getCollegesByRegion(RegionAdministrative region) {
		List<College> allColleges = getColleges(); 
		List<College> colleges = allColleges.stream().filter(c -> c.getRegion().equals(region)).collect(Collectors.toList()); 
		return colleges;
	}
	...
	public List<SchoolBoard> getSchoolBoards() {
		...
		List<SchoolBoard> schoolBoards = organizations.stream().filter(p).map(o -> (SchoolBoard)o).collect(Collectors.toList()); 
		return schoolBoards;
	}
	...
	public List<SchoolBoard> getSchoolBoardsByRegion(RegionAdministrative region) {
		...
		List<SchoolBoard> schoolBoards = allSchoolBoards.stream().filter(sb -> sb.getRegion().equals(region)).collect(Collectors.toList()); 
		return schoolBoards;
	}
	
	public List<School> getSchoolPrivateSchoolByRegion(RegionAdministrative region) {
		List<Organization> organizations = getAllOrganizations(); 
		Predicate<Organization> p = o -> (o instanceof School) && o.getRegion().equals(region) && "".equals(o.parentCode); 
		List<School> schools = organizations.stream().filter(p).map(o -> (School)o).collect(Collectors.toList()); 
		return schools;
	}
	
	public Organization findNearestSchoolFrom(PostalCode postalCode) {
		List<Organization> organizations = getAllOrganizations(); 
		List<String> postalCodes = organizations.stream().map(s -> s.getPostalCode()).collect(Collectors.toList()); 
	...
	private Organization findSchoolByPostalCode(List<Organization> quebecSchools, String postalCode) {
		Organization school = quebecSchools.stream().filter(s -> s.getPostalCode().equals(postalCode)).findFirst().get(); 
		return school;
	}
