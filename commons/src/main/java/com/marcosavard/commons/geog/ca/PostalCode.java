...
public class PostalCode implements Serializable {
  ...
  public enum Region {
    NEWFOUNDLAND, NOVA_SCOTIA, PEI, NEW_BRUNSWICK, GASPESIE, COTE_NORD, SAGUENAY, CAPITALE_NATIONALE, CHAUDIERE_APPALACHES, MAURICIE, CENTRE_DU_QUEBEC, ESTRIE, MONTEREGIE, MONTREAL, LANAUDIERE, LAURENTIDE, OUTAOUAIS, ABITIBI, OUEST_DU_QUEBEC, NORD_DU_QUEBEC, ONTARIO_EASTERN, TORONTO, ONTARIO_CENTRAL, ONTARIO_WESTERN, ONTARIO_NORTHERN, MANITOBA, SASKATCHEWAN, ALBERTA_SOUTHERN, ALBERTA_CENTRAL, ALBERTA_NORTHERN, BRITISH_COLUMBIA_MAIN_LAND, BRITISH_COLUMBIA_VANCOUVER_ISLAND, NUNAVUT, NWT, YUKON,
  };
  ...
  public static PostalCode of(String text) {
    PostalCode postalCode = new PostalCode(text);
    return postalCode;
  }
  ...
  private Region getRegionEasternQuebecUrban() {
    ...
    if (_postalCode.startsWith("G8P")) {
      ...
      region = Region.SAGUENAY;
    } else if ((compareTo(new PostalCode("G7G1A1")) >= 0)
        && (compareTo(new PostalCode("G8P1A1")) < 0)) {
      region = Region.SAGUENAY;
    } else if (compareTo(new PostalCode("G8T1A1")) >= 0) {
      region = Region.MAURICIE;
    } else if ((compareTo(new PostalCode("G4R1A1")) >= 0)
        && (compareTo(new PostalCode("G5T1A1")) < 0)) {
