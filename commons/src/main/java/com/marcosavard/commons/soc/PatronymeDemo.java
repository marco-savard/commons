...
public class PatronymeDemo {
	...
	private static void getCommonPatronymes() {
		...
		Predicate<Patronyme> pred = p -> StringUtil.startsWith(p.getName(), "W"); 
		...
		String regex = "St-.*"; 
		List<Patronyme> hagionymes = patronymes.stream().filter(p -> p.getName().matches(regex)).collect(Collectors.toList());
	...
	private static void findClosestPatronymes() {
		String[] mispelledNames = new String[] {
			"Tramblay", 
			"Marin", 
			"Gautier", 
			"Peletier", 
