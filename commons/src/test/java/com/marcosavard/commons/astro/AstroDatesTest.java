...
public class AstroDatesTest {
  ...
  @Test
  public void testJulianZonedDateUnixEra() {
    LocalDateTime unixEra = LocalDateTime.of(1970, Month.JANUARY.getValue(), 1, 0, 0, 0);
    testJulianDate(unixEra, 2440587.5);
  }

  @Test
  public void testJulianZonedDateY2K() {
    LocalDateTime y2k = LocalDateTime.of(2000, Month.JANUARY.getValue(), 1, 18, 0, 0);
    testJulianDate(y2k, 2451545.25);
  }
  ...
  private void testJulianDate(LocalDateTime localDateTime, double expected) {
    ZoneId utc = ZoneOffset.UTC;
    double delta = 0.001;
    ZonedDateTime zonedDateTime = ZonedDateTime.of(localDateTime, utc);

    double result = AstroDates.toJulianDay(zonedDateTime);
