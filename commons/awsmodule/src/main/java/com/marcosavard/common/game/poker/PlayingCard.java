package com.marcosavard.common.game.poker;
...
public class PlayingCard implements Comparable<PlayingCard> {
    ...
    public static List<PokerHand> arrangeByPokerHands(List<PlayingCard> cards) {
        List<PokerHand> arrangedCards = new ArrayList<>();
        List<PlayingCard> remaining = new ArrayList<>(cards);

        for (int i=0; i<5; i++) {
            PokerHand hand = PlayingCard.findHigherHand(remaining);
            arrangedCards.add(hand);
            remaining.removeAll(hand.getCards());
        }

        for (PokerHand hand : arrangedCards) {
